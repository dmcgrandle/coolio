...
class EnvStateMachine(StateMachine):
    states = States.from_enum(
        EnvironmentState, initial=EnvironmentState.COOL, final=EnvironmentState.OFF)
    ...
    def change_fan_speed(self, new_speed):
        with db.app.app_context():
            ...
            current_app.pwm[self.auto.fan.pwm_channel].change_duty_cycle(
                new_speed)
            db.session.commit()
