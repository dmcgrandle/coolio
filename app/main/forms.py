...
def set_fan_and_sensor_from_obj(form, auto):
    form.temp_sensor_name.choices = [
        sensor.name for sensor in TempSensor.query.all()]
...
class AutomationForm(FlaskForm):
    ...
    edit = SubmitField('Edit automation', render_kw={
                       'class': 'btn btn-outline-warning'})
    delete = SubmitField('Delete automation', render_kw={
                         'class': 'btn btn-outline-danger'})
    cancel = SubmitField('Cancel', render_kw={
                         'class': 'btn btn-outline-secondary'})
    save = SubmitField('Save automation', render_kw={
